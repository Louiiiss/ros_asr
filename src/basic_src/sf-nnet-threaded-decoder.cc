...
void SFNnetThreadedDecoder::timeout() {
...
void SFNnetThreadedDecoder::finishDecoding() {
	using namespace fst;
	...
	nnet_decoder -> GetLattice(end_of_utterance, &clat, NULL);
	//clat.Write(//cout);
	...
	//if (keyphrase_activated && !robotSpeaking) {
		//nnet_decoder -> GetAdaptationState(adaptation_state);
	...
	//}
	//	BaseFloat inv_acoustic_scale = 1.0 / config->nnet2_decoding_config.acoustic_scale;
...
void SFNnetThreadedDecoder::decode(Vector<BaseFloat>* data) {
	//cout << "nnet listener decode " << data->Dim() << endl;
	count++;
	using namespace fst;
	...
	if (fresh_utterance) {
		...
		nnet_decoder = new SingleUtteranceNnet2DecoderThreaded(config->nnet2_decoding_config,
		               *(config->trans_model),
		               *(config->nnet),
		               *(config->decode_fst),
		               *(config->feature_info),
		...
		controller->my_log("SingleUtterance finished");
	...
	//cerr << "freq " << samp_freq << endl;
	//cout << "decode data " << data -> Dim() << endl;
	//controller->my_log("Accepting waveform");
	

	nnet_decoder -> AcceptWaveform(samp_freq, *data);



	//controller->my_log("Finished accepting waveform");
	//nnet_decoder -> AdvanceDecoding();

	if (nnet_decoder -> EndpointDetected(config->endpoint_config)) {
		nnet_decoder -> TerminateDecoding();
		finishDecoding();
	}
#
}
